...
掲載可能Evidenceには各段階の短い連番IDを付け、すべてのIDが公開項目または限定された
除外理由へ一度だけ割り当てられたかを確認する。AIが項目を返さなかった時に薄い文章を
機械生成せず、その編集結果を不合格にする。単純な事実要約は`GPT-4o mini`を使う。
...
本文が長いだけでは昇格しない。quality modelが単一Evidenceでも内容契約を満たせない時だけ、
