...
`GPT-4.1`、単純な事実要約は`GPT-4.1 mini`を使う。分析、表・グラフ中心、高数値密度の
本文は設定済みquality modelへ最初から送り、下位modelの出力が契約を満たさない場合だけ一度昇格する。
本番でHTTP 413/400を返すmodelは提供中でも日次経路に使わず、同一契約を安定して満たす
最上位の稼働確認済みmodelをquality modelとする。
本文が長いだけでは昇格せず、quality modelから低品質modelへ降格しない。
