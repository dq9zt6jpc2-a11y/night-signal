...
- Editor前は日付、取得成否、カテゴリ主体、ナビゲーション除外などの構造条件だけを
  判定する。特定の変化語や過去の失敗事例語で掲載候補を落とさない。
...
Editorは二つの意味判断を直列に行う。最初に短い根拠表示で全候補IDをレビューし、
各IDを掲載イベント群または限定された除外理由へ一度だけ割り当てる。同一事象の
複数記事はここで統合する。掲載数の上限やカテゴリごとの少数選抜は設けず、不確実な
重要候補は掲載側へ残す。次に掲載イベント群だけの参照元本文全体を把握し、次を作る。
...
モデルへは、最大8,000字まで保持した本文、短いEvidence ID、Evidenceごとのwatch topic IDだけを渡す。
...
時に薄い文章を機械生成せず、その編集結果を不合格にする。全候補の変化点レビューは
`GPT-4.1 mini`、単純な事実要約は`GPT-4o mini`を使う。分析、表・グラフ中心、高数値密度の本文は
`GPT-5 mini`へ最初から送り、通常modelの出力が契約を満たさない場合だけ一度昇格する。
本番でHTTP 413/400を返すmodelは提供中でも日次経路に使わず、同一契約を安定して満たす
最上位の稼働確認済みmodelをquality modelとする。
本文が長いだけでは昇格しない。API障害時のfallbackも同じ決定的検証を通過した出力だけを採用する。
...
- 既公開なら即終了する。
- 既公開号を強制再収集する経路は持たない。手動回復で日付を明示する場合もJST当日だけを
  受け付け、過去号を最新号として公開する例外をworkflowに持たない。
